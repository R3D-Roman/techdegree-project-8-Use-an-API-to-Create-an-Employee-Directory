...
function generateHTML(data) {
  ...
  data.map(result => {
    ...
    divgallery.innerHTML = `  
        ...
        <p class="address">${result.location.street.number} ${
...
function left() {
  ...
  let left =
    getPrevSibling(current, ".gallery.show") ||
    getLastSibling(current, ".gallery.show");
  popup(left.id);
}
...
function right() {
  ...
  if (!current) return false;
  let right =
    getNextSibling(current, ".gallery.show") ||
    getFirstSibling(current, ".gallery.show");
  popup(right.id);
}
...
input.addEventListener("keyup", () => {
  ...
  for (let i = 0; i < col.length; i++) {
    ...
    if (
      namesText.toUpperCase().indexOf(inputValue) > -1 ||
      userNameText.toUpperCase().indexOf(inputValue) > -1
    ) {
      col[i].style.display = "";
